...
const DEFAULT_PIPER_VOICE = "en_US-lessac-low";
...
class Speaker {
   ...
   * @param {string} opts.voice provider-qualified default voice
  ...
  constructor(opts) {
    this.log = opts.log;
    this.defaultVoice = normalizeVoiceId(opts.voice) || {
      provider: "piper",
      voice: DEFAULT_PIPER_VOICE,
    };
    this.voice =
      this.defaultVoice.provider === "piper"
    ...
    if (opts.azure && opts.azure.enabled) {
      const azure = new AzureTts({
        ...
        voice:
          this.defaultVoice.provider === "azure"
            ? this.defaultVoice.voice
            : DEFAULT_AZURE_VOICE,
      });
  ...
  stats() {
    return {
      status: "ok",
      voice: formatVoiceId(this.defaultVoice),
      backend: this.defaultVoice.provider,
    ...
    try {
      ...
      this.log.info(
        `pipo-speak: spoke ${text.length} chars at volume ${vol}` +
          ` with ${prepared.provider} voice "${prepared.voice}"` +
  ...
  async _playFileNow(filePath, opts) {
    ...
    try {
      ...
      if (this.warm && this.warm.isReady()) {
      ...
      const ext = path.extname(playbackPath).toLowerCase() || ".wav";
      route.outName = `pipo-speak-soundboard${ext}`;
  ...
  _renderers(voice) {
    const requested = normalizeVoiceId(voice) || this.defaultVoice;
    const choices = [
      requested,
      this.defaultVoice,
      { provider: "azure", voice: this.azureVoice || DEFAULT_AZURE_VOICE },
      { provider: "piper", voice: this.voice },
    ];
    const seen = new Set();
    const renderers = [];
    for (const choice of choices) {
      const id = formatVoiceId(choice);
      ...
      if (choice.provider === "azure") {
        renderers.push({
          ...
          synthesize: (text, lengthScale) => {
            if (!this.azure) {
              throw new Error(
                "Azure is not configured (no usable subscription key)",
              );
            }
            return this.azure.synthesize(text, choice.voice, lengthScale);
          },
        });
      } else if (choice.provider === "piper") {
        renderers.push({
          provider: "piper",
          voice: choice.voice,
          ...
          synthesize: async (text, lengthScale) => {
            this._gate();
            await this._ensureVoice(choice.voice);
            return this._synthesize(text, choice.voice, lengthScale);
          },
        });
      }
    }
  ...
  async _generateFirstAvailable(
    ...
    for (let index = 0; index < renderers.length; index++) {
      ...
      try {
        if (useCache) {
          const cached = this.cache.getBest(text, lengthScale, renderer);
          if (cached) {
            return { ...cached, fromCache: true };
          }
        }
        return {
          ...(await this._generate(text, lengthScale, renderer)),
          fromCache: false,
        };
      } catch (err) {
        lastError = err;
        const next = renderers[index + 1];
        this.log.warn(
          `pipo-speak: ${renderer.provider} voice "${renderer.voice}" failed ` +
            `(${err.message}); ${next ? `trying ${next.provider} voice "${next.voice}"` : "no fallback voice remains"}.`,
        );
      }
    }
...
function normalizeVoiceId(value) {
  ...
  const voice = value.slice(separator + 1);
  if ((provider !== "azure" && provider !== "piper") || !voice) {
    return null;
  }
  return { provider, voice };
}

function formatVoiceId(value) {
  return value ? `${value.provider}:${value.voice}` : "default";
}
