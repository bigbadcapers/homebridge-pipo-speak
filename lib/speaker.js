...
class Speaker {
  ...
  async _primeNow(rawText, opts) {
    ...
    const cached = this.cache.getBest(
      text,
      lengthScale,
      this._cacheCandidates(voice),
    );
    if (cached) {
      this._scheduleUpgrade(text, lengthScale, cached);
      return { code: 200, message: "already cached" };
    }
    let artifact;
    try {
      artifact = await this._renderArtifact(text, voice, lengthScale);
      ...
      if (artifact) {
        fs.promises.unlink(artifact.path).catch(() => {});
      }
      if (this.cooldownMs > 0) {
        await delay(this.cooldownMs);
      }
  ...
  async _prepareWav(text, voice, lengthScale) {
    ...
    const artifact = await this._renderArtifact(text, voice, lengthScale);
    await this.cache.put(
      artifact.path,
      text,
      artifact.voice,
    ...
    return { path: artifact.path, temp: true, fromCache: false };
  }

  _cacheCandidates(voice) {
    const candidates = [];
    if (this.azureVoice) {
      candidates.push({
    ...
    candidates.push({
      voice: voice && voice.includes("_") ? voice : this.voice,
      provider: "piper",
  ...
  /** Start one best-effort cloud promotion without delaying cached playback. */
  _scheduleUpgrade(text, lengthScale, cached) {
    if (!this.azure || cached.quality >= AZURE_QUALITY) {
      return;
    }
    ...
    const upgrade = this.azure
      .synthesize(text, this.azureVoice, lengthScale)
      .then(async (wav) => {
        try {
          const stored = await this.cache.put(
            ...
            this.azureVoice,
            lengthScale,
            { provider: "azure", quality: AZURE_QUALITY },
          );
          if (stored) {
            this.log.info(
              `pipo-speak: quietly upgraded cached phrase to Azure voice (${this.azureVoice}).`,
            );
          }
        } finally {
  ...
  async _renderArtifact(text, voice, lengthScale) {
    if (this.azure) {
      try {
        return {
          path: await this.azure.synthesize(text, voice, lengthScale),
          voice,
          provider: "azure",
          quality: AZURE_QUALITY,
        };
    ...
    return {
      path: await this._synthesize(text, voice, lengthScale),
      voice,
      provider: "piper",
      quality: PIPER_QUALITY,
    };
