...
class Speaker {
  ...
  async _primeNow(rawText, opts) {
    ...
    let wav;
    try {
      wav = await this._render(text, voice, lengthScale);
  ...
  async _prepareWav(text, voice, lengthScale) {
    ...
    const wav = await this._render(text, voice, lengthScale);
    await this.cache.put(wav, text, voice, lengthScale);
  ...
  async _render(text, voice, lengthScale) {
    if (this.azure) {
      try {
        return await this.azure.synthesize(text, voice, lengthScale);
      } catch (err) {
        this.log.warn(
    ...
    return this._synthesize(text, voice, lengthScale);
  }
