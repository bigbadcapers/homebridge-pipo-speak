...
function normalizeSoundFiles(soundFiles) {
...
/**
 * Depth-first scan of `rootDir` for playable audio files.
 *
 * When `soundFiles` is supplied, only those files are considered, in the order
 * configured. Otherwise entries in each directory are visited in
 * case-insensitive name order; a subdirectory is descended into the moment it is
 * reached (true depth-first), so the result mirrors what a person reading the
 * tree top-to-bottom would pick.
 ...
 * @param {string[]} [opts.soundFiles] explicit relative paths under `rootDir`
...
function scanSounds(rootDir, opts = {}) {
  ...
  const soundFiles = normalizeSoundFiles(opts.soundFiles);

  if (soundFiles.length > 0) {
    const seen = new Set();
    for (const soundFile of soundFiles) {
      const full = path.resolve(root, soundFile);
      if (seen.has(full) || !isInsideRoot(root, full)) {
        continue;
      }
      seen.add(full);
      let stat;
      ...
      if (stat.isFile() && isAudioFile(full)) {
        found.push({
